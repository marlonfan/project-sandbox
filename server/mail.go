...
func buildVerificationMail(cfg SMTPConfig, to, code string) string {
	...
	spacedCode := ""
	for _, ch := range code {
		spacedCode += string(ch) + " "
	}
	spacedCode = strings.TrimRight(spacedCode, " ")
	html := `<!DOCTYPE html>
<html lang="zh-CN">
...
<table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;background:#ffffff;border:1px solid #ece9e0;border-radius:14px;font-family:-apple-system,BlinkMacSystemFont,'PingFang SC','Hiragino Sans GB','Microsoft YaHei',sans-serif;">
  ...
  <tr><td align="center" style="padding:26px 44px 8px;">
    <div style="background:#faf9f5;border:1px solid #ece9e0;border-radius:12px;padding:20px 36px;font-size:40px;font-weight:700;letter-spacing:12px;padding-left:24px;color:#1f1e1d;">` + spacedCode + `</div>
  </td></tr>
  <tr><td style="padding:14px 44px 36px;">
    <p style="margin:0;font-size:13px;color:#8a867e;line-height:1.9;">验证码 15 分钟内有效，请尽快填写。<br>若非本人操作，请忽略此邮件，你的邮箱不会因此被注册。</p>
  </td></tr>
  <tr><td style="padding:16px 44px;background:#faf9f5;border-top:1px solid #f0eee6;border-radius:0 0 14px 14px;">
