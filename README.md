...
以「变化为中心」的周回顾层，与沙盘列表共享同一份 state（列表管「记」，周视图管「想」）。顶栏「沙盘 / 本周」切换，本地与服务端模式均可用。

- **自动推导**：新增事项（createdAt）、完成（resolvedAt）、进展（notes 时间戳）、状态变化（statusLog）按自然周（周一–周日）聚合；上周可切换回看
- **需要关注**：受阻项目 + 逾期项目置顶，卡片上可直接「改为进行中」「顺延一周」
- **无动静名单**：本周零活动的未完成项目按沉睡天数排序，可「暂停」或「设个期限」
- **轻操作**：每条进展类变化可「补一句」，走同一自动保存通道
- **一键周报**：按变化自动生成纯文本周报，复制即用
- 数据模型增量：`issue.resolvedAt`、`project.statusLog`（normalize 兼容旧数据，旧完成的的事项无 resolvedAt 不参与「完成」推导，重新勾选一次即可）

### 主视图筛选与保存指示（2026-08-29 加入）

- 顶部统计胶囊全部变为**可点筛选入口**：进行中 / 风险中（含受阻）/ 即将到期（7 天内 + 逾期）/ 未完成事项，点击筛出对应项目、再点取消，与搜索和状态筛选叠加生效
- 新增「**即将到期 N**」胶囊（红色 = 有临期/逾期），补上时间维度
- 保存指示器：右上角实时显示 保存中… / 已保存 HH:MM / 保存失败（红色，点击重试）
- 兜底保存：页面隐藏（切标签、手机切后台）、关闭前自动把防抖中的改动立即发出，不再依赖 0.5 秒窗口

### 全局快添（2026-08-29 加入）

`Cmd/Ctrl+K`（或工具栏「快速记录」按钮、移动端右下角 ＋ 悬浮球）随时呼出一句话捕获：

- **猜测 + 确认制**：本地字符 bigram 评分实时猜测目标（项目名/事项标题字面重叠 + 最近活跃加权），
  高置信实线芯片、低置信虚线标注「猜的」——不自动落库，看一眼回车即存，猜错点一下更换
- **类型自动判断**：文本与某条未完成事项标题高度重合（≥45 分）→ 自动切「进展记录」挂到该事项，否则默认「新事项」
- 中文无分词，纯前端实现零依赖；未来可替换为 LLM 猜测引擎，确认制交互不变
